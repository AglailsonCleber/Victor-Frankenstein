...
async function fetchData(searchType, query, page, searchMode) {
    ...
    switch (searchType) {
        case 'movie': return searchMovieByTitle(query, page);
        case 'tv': return searchTvByTitle(query, page);
        case 'person': return searchPersonByName(query, page);
...
async function startPagination(interaction, query, searchType, searchMode = 'title') {
    ...
    try {
        ...
        // --- Função Central para Buscar e Atualizar o Estado (Seu código, mantido) ---
        const fetchAndRender = async (pageToFetch) => {
            const data = await fetchData(searchType, query, pageToFetch, searchMode);

            apiResults = data.results;
            // Validação para garantir que current_page não é 0
        ...
        const renderMessage = (currentItem, currentIdx, totalResOnPage, currentPg, totalPg) => {
            ...
            // 1. Botões de Navegação de Resultados (Filmes/Séries/Pessoas na página)
            ...
            return {
                content: content,
                embeds: [embed],
                // (NOVO) Retorna 4 fileiras de botões
        ...
        // --- Collector e Listeners (MODIFICADO) ---
        const filter = i => i.user.id === interaction.user.id; 
        const collector = reply.createMessageComponentCollector({ filter, time: 900000 }); // 15 minutos

        collector.on('collect', async i => {
            // Adia a interação do botão principal (exceto para os que abrem modais ou dão reply)
            // Não damos deferUpdate() aqui ainda, pois alguns botões dão reply/showModal.
            
            const customId = i.customId;

            // --- LÓGICA DOS NOVOS BOTÕES ---

            // 1. (NOVO) Pular para Página
            if (customId === BTN_ID_JUMP_TO_PAGE) {
                const modal = new ModalBuilder()
                    .setCustomId(MODAL_ID_JUMP) // ID estático, vamos pegar a resposta com awaitModalSubmit
                ...
                // Espera o usuário enviar o modal (1 minuto de tempo)
                const submitted = await i.awaitModalSubmit({
                    filter: mi => mi.user.id === i.user.id && mi.customId === MODAL_ID_JUMP,
                    time: 60000 
                }).catch(() => null); // Retorna null se o tempo esgotar

                if (submitted) {
                    const pageStr = submitted.fields.getTextInputValue(MODAL_INPUT_ID_JUMP);
                    const pageNum = parseInt(pageStr);

                    if (!isNaN(pageNum) && pageNum > 0 && pageNum <= totalPages) {
                        // Busca a nova página
                        const newRender = await fetchAndRender(pageNum);
                        // Atualiza a mensagem original usando a interação do *modal*
                        await submitted.update(newRender); 
                    } else {
                        // Dá um feedback efêmero sobre o erro
                        await submitted.reply({ content: `❌ Página inválida. O número deve estar entre 1 e ${totalPages}.`, ephemeral: true });
                    }
                }
            ...
            if (customId === BTN_ID_PROVIDERS) {
                ...
                try {
                    const providerEmbed = await getWatchProviders(searchType, currentItem.id, currentItem.title);
                    await i.editReply({ embeds: [providerEmbed] });
                } catch (error) {
                    await i.editReply({ content: `❌ Erro ao buscar *Watch Providers*: ${error.message}` });
                }
                return; // Impede que o código de paginação normal rode
            }
            ...
            if (customId === BTN_ID_PUBLISH) {
                const currentItem = apiResults[currentResultIndex];
                // Recria o embed e o content do item atual
                const { content, embed } = formatEmbedAndContent(searchType, currentItem, currentResultIndex, apiResults.length, currentPage, totalPages);
                ...
                await i.reply({ content: '✅ Resultado publicado no canal!', ephemeral: true });
                return; // Impede que o código de paginação normal rode
            }
